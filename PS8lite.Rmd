...
1. Why didn't I apply the same change to the `test` set? There is no "SalePrice" variable in the 'test' set. 
1. If I transformed the outcome variable now, what will I have to make sure to do later on? You have to "de-transform" the variable so it aligns with your desired original outcome variable. 
...
```{r}
test <- test %>%
  mutate(logSalePrice_hat = predict(object = model_rf, newdata = test),
